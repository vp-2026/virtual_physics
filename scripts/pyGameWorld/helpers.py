...
def areaForPoly(verts):
    area = 0
    # Convert each vertex list/tuple into Vec2d(x, y)
    pmv = [pm.Vec2d(v[0], v[1]) for v in verts]
...
def segs2Poly(seglist, r):
    # Convert each point in seglist to Vec2d(x, y)
    ...
    for i in range(1, len(vlist) - 1):
        ...
        # Construct the unit vector directly instead of setting .angle
...
def polyValidate(verts):
    for i in range(len(verts)):
        ...
        if _vcross2(bx - ax, by - ay, cx - bx, cy - by) > 0:
            return False
    return True
...
def word2Color(colorname):
    ...
    try:
        ...
        if c == 'blue':
            return (0, 76, 153, 255)
        if c == 'lightblue':
            return (130, 200, 230, 255)
        elif c == 'red':
            return (200, 100, 100, 255)
        elif c == 'green':
            return (100, 180, 100, 0)
        elif c == 'pink': 
            return (230, 0, 200, 255)
        elif c == 'orange':              
...
def filterCollisionEvents(eventlist, slop_time=0.2):
    begin_list = {}
    last_list = {}
    col_list = {}
    col_list_beg = {}
    ...
    for o1, o2, tp, tm, ci in eventlist:
        if o2 < o1:
            ...
            new_cins = []
            for n in ci[0]:
                # new_cins.append({'x': -n['x'], 'y': -n['y']}) - modified 1.16
                # If normal isn't a dict, just skip flipping it (or treat as 0-vector)
                if isinstance(n, dict):
                    new_cins.append({'x': -n.get('x', 0.0), 'y': -n.get('y', 0.0)})
                else:
                    new_cins.append({'x': 0.0, 'y': 0.0})
            ci[0] = new_cins

        comb = re.sub('__', '_', o1 + "_" + o2).strip('_')
        if tp == 'begin':
