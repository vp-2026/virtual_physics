...
def _draw_obj(o, s, makept, lighten_amt=0):
    ...
    #         pg.draw.polygon(s, acolor, garea)
        ...
        _draw_container_outline(o, s, makept, lighten_amt)
...
def _draw_container_fill(o, s, makept, lighten_amt=0):
    # translucent inner area
    garea = [makept(p) for p in o.vertices]
    if o.inner_color is not None:
        acolor = (o.inner_color[0], o.inner_color[1], o.inner_color[2], 128)
        acolor = _lighten_rgb(acolor, lighten_amt)
...
def _draw_container_outline(o, s, makept, lighten_amt=0):
    # black border / outer walls
    col = _lighten_rgb(o.outer_color, lighten_amt)
    for poly in o.polys:
        vtxs = [makept(p) for p in poly]
        pg.draw.polygon(s, col, vtxs)
...
def drawWorld(world, backgroundOnly=False, lightenPlaced=False):
    ...
    # Pass 0: blockers as before
    for b in world.blockers.values():
        drawpts = [makept(p) for p in b.vertices]
        pg.draw.polygon(s, b.color, drawpts)

    # Pass 1: draw container *fills* in the background
    for o in world.objects.values():
        if o.type == 'Container':
            _draw_container_fill(o, s, makept)

    # Pass 2: draw all objects (don’t re-draw container fill here)
    for o in world.objects.values():
    ...
    # Pass 3: draw container *outlines* on top so borders are crisp
    for o in world.objects.values():
        if o.type == 'Container':
            _draw_container_outline(o, s, makept)
