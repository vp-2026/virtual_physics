...
def build_three_ball_tools(color_obj1: str = "lightblue", color_obj2: str = "orange", color_obj3: str = "pink") -> Dict[str, dict]:
    return {
        "obj1": {
...
def normalize_trial_name(name: str) -> str:
    ...
    if normalized in ALLOWED_TRIALS:
        return normalized
    if not normalized.endswith("_EY"):
        candidate = f"{normalized}_EY"
        if candidate in ALLOWED_TRIALS:
            return candidate
