...
class PGWorld(object):
    ...
    def addContainer(
        self, name, ptlist, width, inner_color, outer_color, density=None, elasticity=None, friction=None
    ):
        assert name not in self.objects.keys(), "Name already taken: " + name
        if density is None:
            density = self.def_density
    ...
    def checkCollision(self, pos, verts):
        """
        Check polygonal collision.  If `verts` is nested (e.g. [ [ [x,y], … ] ] ), take the first level.
        Then translate each vertex by `pos` before querying.
        """
        # unwrap one level if necessary:
        if len(verts) > 0 and isinstance(verts[0][0], (list, tuple)):
            poly = verts[0]
        else:
            poly = verts

        # now convert to floats + translate by pos
        nvert = []
        for v in poly:
            x = float(v[0]) + pos[0]
            y = float(v[1]) + pos[1]
        ...
        # Step a tiny amount so the shape actually gets registered
        self._cpSpace.step(1e-6)
    ...
    def checkCircleCollision(self, pos, rad):
        """
        Check circular collision at `pos` with radius `rad`.
        """
        tmpBody = pm.Body(1, 1)
    ...
    def distanceToGoal(self, point):
        ...
        if type(self.goalCond) == PGCond_SpecificTouch:
            ...
            return np.abs(o1.distanceFromPoint([0, 0]) - o2.distanceFromPoint([0, 0]))
        else:
    ...
    def distanceToGoalContainer(self, point):
        """
        For container objects, this returns distance to the top of the container.
        """
        assert self.goalCond, "Goal condition must be specified to get distance"
        # Special case... requires getting two distances
        if type(self.goalCond) == PGCond_SpecificTouch:
    ...
    def toDict(self):
        ...
        for nm, o in self.objects.items():
            attrs = dict(type=o.type, color=list(o.color), density=o.density, friction=o.friction, elasticity=o.elasticity)
            if o.type == "Poly":
        ...
        if self.goalCond is None:
            ...
            if gc.type == "AnyInGoal":
                wdict["gcond"] = {"type": gc.type, "goal": gc.goal, "obj": "-", "exclusions": gc.excl, "duration": gc.dur}
...
def loadFromDict(d):
    ...
    # if d["gcond"] is not None:
        ...
        if g["type"] == "AnyInGoal":
            ...
            raise Exception("Invalid goal condition type given")
