...
class PGObject(object):
    ...
    def distanceFromPoint(self, point):
        if hasattr(point, "x") and hasattr(point, "y"):
            ...
            qp = pm.Vec2d(float(point[0]), float(point[1]))
        info = self._cpShape.point_query(qp)
...
class PGPoly(PGObject):
    ...
    def __init__(self, name, space, vertices,
                 density=DEFAULT_DENSITY, elasticity=DEFAULT_ELASTICITY,
...
class PGBall(PGObject):
    ...
    def __init__(self, name, space, position, radius,
                 density=DEFAULT_DENSITY, elasticity=DEFAULT_ELASTICITY,
                 friction=DEFAULT_FRICTION, color=DEFAULT_COLOR):
    ...
    rotation = property(getRot, PGObject.setRot)
...
class PGSeg(PGObject):
    ...
    def __init__(self, name, space, p1, p2, width,
        ...
        if mass == 0:
            ...
            pos = pm.Vec2d((p1[0] + p2[0]) / 2.0, (p1[1] + p2[1]) / 2.0)
            v1 = pm.Vec2d(p1[0], p1[1]) - pos
            v2 = pm.Vec2d(p2[0], p2[1]) - pos
...
class PGContainer(PGObject):
    ...
    def __init__(self, name, space, ptlist, width,
        ...
        # Convert each vertex into Vec2d(x, y)
        ...
        if self.isStatic():
            ...
            for i in range(len(self.polylist)):
                ...
                for j in range(len(self.polylist[i])):
                    vj = self.polylist[i][j]
                    # Some container polys are stored as tuples/lists, not Vec2d.
                    # Normalize before rotating so dynamic containers render correctly.
                    if hasattr(vj, "rotated"):
                        v_local = vj
                    else:
                        v_local = pm.Vec2d(vj[0], vj[1])
                    v_world = v_local.rotated(rot) + pos
                    tpol.append(np.array(v_world))
                polys.append(tpol)
        return polys
    ...
    def pointIn(self, p):
        v = pm.Vec2d(p[0], p[1])
        info = self._cpSensor.point_query(v)
        if hasattr(info, "distance"):
    ...
    def distanceFromPoint(self, point):
        if hasattr(point, "x") and hasattr(point, "y"):
            ...
            qp = pm.Vec2d(float(point[0]), float(point[1]))
        info = self._cpSensor.point_query(qp)
        if hasattr(info, "distance"):
            return info.distance
        if isinstance(info, (list, tuple)):
            return info[0]
...
class PGCompound(PGObject):
    ...
    def __init__(self, name, space, polygons,
                 density=DEFAULT_DENSITY,
        ...
        self.polylist = []
        self._cpShapes = []
        # If it's static, add polygons in place
        if density == 0:
            polyCents = []
            areas = []
            ...
            # Build shapes and compute total moment of inertia
            imom = 0
            for pc, a, verts in zip(polyCents, areas, polygons):
                ...
                # Convert each vertex to Vec2d offset by pos
            ...
            self._cpBody = pm.Body(mass, imom)
            # Assign body to each shape, but do not add shapes yet
            for sh in self._cpShapes:
                ...
                sh.collision_type = COLTYPE_SOLID
                sh.name = name
            # Add body and all shapes to the space together
            space.add(self._cpBody, *self._cpShapes)
            self._cpBody.position = loc
    ...
    def distanceFromPoint(self, point):
        ...
        dists = []
        for s in self._cpShapes:
            info = s.point_query(qp)
            if hasattr(info, "distance"):
                dists.append(info.distance)
            elif isinstance(info, (list, tuple)):
                dists.append(info[0])
            else:
                dists.append(float(info))
        return min(dists)
...
class PGGoal(PGObject):
    ...
    def pointIn(self, p):
        v = pm.Vec2d(p[0], p[1])
        info = self._cpShape.point_query(v)
        if hasattr(info, "distance"):
            return info.distance <= 0
        if isinstance(info, (list, tuple)):
            return info[0] <= 0
